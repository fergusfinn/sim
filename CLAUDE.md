...
```bash
cargo build --release
./target/release/sim --config configs/config.toml
```
...
│   └── qwen3_30b_a3b.toml  # Qwen benchmark config
...
- `configs/config.toml` - Default config (H100, Llama-3-70B, Poisson arrivals)
- `configs/test_blog.toml` - Blog benchmark (H100 TP=2, closed-loop, 64 users)
- `configs/qwen3_30b_a3b.toml` - Qwen benchmark config
